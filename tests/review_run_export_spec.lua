...
describe("review run export", function()
	it("creates an agent-attribution checkpoint for trusted handoffs", function()
		...
		t.assert_match(exported, "### Required workflow", "protocol should group completion steps")
		t.assert_match(exported, "### Manifest", "protocol should group manifest requirements")
		t.assert_match(exported, "### Regions", "protocol should group range guidance")
		t.assert_match(exported, "### Outcomes", "protocol should group outcome definitions")
		...
		t.assert_match(exported, "Derive one%-based old/new ranges from the authoritative zero%-context diff", "protocol should explain how to map diff hunks to regions")
		t.assert_match(exported, "When a side has count `0`, omit that side's range", "protocol should explain insertion and deletion ranges")
		t.assert_eq(exported:find('"type":"modified"', 1, true), nil, "protocol example should not request unused change types")
		t.assert_eq(exported:find('"description":', 1, true), nil, "protocol example should not request unused change descriptions")
		t.assert_match(exported, "Write `summary` as a concise, direct response", "protocol should request reviewer-facing answers")
		t.assert_match(exported, "Do not narrate investigation steps", "protocol should discourage process narration")
		t.assert_match(exported, "`answered`: responded without code changes; directly answer the feedback", "answered claims should receive direct responses")
		t.assert_match(exported, "do not repeat these claim%-by%-claim results", "protocol should keep the conversational response concise")
		...
		local absolute_diff_path = vim.fs.joinpath(root, ".doubt", "runs", run_id, "diff")
		...
		t.assert_eq(exported:find(absolute_diff_path, 1, true) ~= nil, true, "protocol should provide the exact diff helper path")
		...
		t.assert_eq(vim.uv.fs_stat(absolute_diff_path) ~= nil, true, "export should generate the diff helper")
		...
		t.assert_eq(run.claims[1].revision, claims.review_revision(file_state.claims[1]), "run metadata should retain the exported claim revision")
		...
		t.assert_match(run.diff_helper_path, "diff$", "run metadata should record its diff helper")
